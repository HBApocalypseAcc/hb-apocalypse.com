...
BOOL XSocket::bAccept(class XSocket * pXSock, unsigned int uiMsg)
{
	...
	// Enable TCP keepalive to prevent NAT/firewall timeouts on accepted connections
	int keepalive = 1;
	setsockopt(pXSock->m_Sock, SOL_SOCKET, SO_KEEPALIVE, (char*)&keepalive, sizeof(keepalive));
