...
void CSprite::PutSpriteFast(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 RECT rcRect;
	...
	if (dX < m_pDDraw->m_rcClipArea.left)
	{
		...
		sx = sx	+ (unsigned short)(m_pDDraw->m_rcClipArea.left - dX);
		szx = szx - (unsigned short)(m_pDDraw->m_rcClipArea.left - dX);
...
//		szx = szx - (m_pDDraw->m_rcClipArea.left - dX);
	...
	{
		//Reparando warning LaloRamos anulado v11 - Agregado (unsigned short)
		sy = sy	+ (unsigned short)(m_pDDraw->m_rcClipArea.top - dY);
		szy = szy - (unsigned short)(m_pDDraw->m_rcClipArea.top - dY);
		if (szy <= 0) {
			m_rcBound.top = -1;
			return;
		}
...
void CSprite::PutSpriteFastWidth(int sX, int sY, int sFrame, int sWidth, DWORD dwTime)
{
 ...
 RECT rcRect;
	...
	if (dX < m_pDDraw->m_rcClipArea.left) 								  
	{
		...
		sx = sx	+ (unsigned short)(m_pDDraw->m_rcClipArea.left - dX); //Agregado (unsigned short)
		szx = szx - (unsigned short)(m_pDDraw->m_rcClipArea.left - dX); //Agregado (unsigned short)
		if (szx <= 0) {
			m_rcBound.top = -1;
			return;
		}
...
//	if (dY+szy > m_pDDraw->m_rcClipArea.bottom-3) return FALSE;
	...
	{
		//Reparando warning LaloRamos anulado v11 - Agregado (unsigned short)
		sx = sx	+ (unsigned short)(m_pDDraw->m_rcClipArea.left+3 - dX);
		szx = szx - (unsigned short)(m_pDDraw->m_rcClipArea.left+3 - dX);
		if (szx < 0) {
			m_rcBound.top = -1;
			return FALSE;
		}
	...
	{
		//Reparando warning LaloRamos anulado v11 - Agregado (unsigned short)
		sy = sy	+ (unsigned short)(m_pDDraw->m_rcClipArea.top+3 - dY);
		szy = szy - (unsigned short)(m_pDDraw->m_rcClipArea.top+3 - dY);
		if (szy < 0) {
			m_rcBound.top = -1;
			return FALSE;
		}
