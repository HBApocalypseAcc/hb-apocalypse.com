...
BOOL XSocket::bBlockConnect(char * pAddr, int iPort, unsigned int uiMsg)
{
 ...
 struct hostent * hp;
	...
	//Reparando warning LaloRamos anulado v11 - cambio "strcpy" por "strcpy_s"
 ...
 int    i, iRet;
	...
	*wp = (WORD)(dwSize + 7);
	...
	for (i = 0; i < (int)(dwSize); i++) {
		...
		m_pSndBuffer[7 + i] = (char)(m_pSndBuffer[7 + i] ^ (KeyS[z] ^ (dwSize - i)));
	...
	if (m_bIsWriteEnabled == FALSE) {
		iRet = _iRegisterUnsentData(m_pSndBuffer, dwSize +7);
	} 
	else iRet = _iSend(m_pSndBuffer, dwSize + 7, TRUE);

	if (iRet < 0) return iRet;
	else return (iRet - 7);
}

int XSocket::iSendMsgServer(char * cData, DWORD dwSize, char cKey)
{
 WORD * wp;
 int    i, iRet;
	...
	*wp = (WORD)(dwSize + 7);
	...
	memcpy((char *)(m_pSndBuffer + 7), cData, dwSize);
	/*for (i = 0; i < (int)(dwSize); i++) {
		m_pSndBuffer[7+i] += (i ^ 49);
		m_pSndBuffer[7+i] = (char)( m_pSndBuffer[7+i] ^ (49 ^ (dwSize - i)) );
	}*/
 ...
 char cKey;
	...
	*pMsgSize = (*wp) - 7;	
	...
	for (i = 0; i < (int)(dwSize); i++) {
		...
		m_pRcvBuffer[7 + i] = (char)(m_pRcvBuffer[7 + i] ^ (KeyS[z] ^ (dwSize - i)));
