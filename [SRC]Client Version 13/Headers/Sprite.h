...
public:
	...
	CSprite(HANDLE hPakFile, class DXC_ddraw * pDDraw, char * cPakFileName, short sNthFile, bool bAlphaEffect = TRUE, bool Pak = true);
	virtual ~CSprite();
	...
	void PutShadowSpriteEight(int sX, int sY, int sFrame, int sEight, DWORD dwTime);
	void PutSpriteFastEight(int sX, int sY, int sFrame, int sEight, DWORD dwTime);
};
